...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
	...
	//Recall fg[0] is reserved for the cost value, 
    ...
    for (unsigned int t = 1; t < N; t++) {
	  ...
	  // use previous actuations if t > 1 (account for latency)
	  if (t > 1) {   
        a = vars[a_start + t - 2];
      ...
      // epsi[t] = psi[t] - psides[t-1] + v[t-1] * delta[t-1] / Lf * d
      
	  // To compute derivatives used `AD<double>` from `CppAD`
      // And passed them to the solver.
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // Set non-actuators upper and lower limits
  ...
  // degrees (values in radians = + or - 0.436332).
